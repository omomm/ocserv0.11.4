...
function tar_lz4_install(){
    ...
    mkdir lz4
	github_lz4_mirror='https://github.com/squidproxy/anyconnect/releases/latest'
	LZ4_VERSION=`curl -s $github_lz4_mirror | sed -n 's/^.*tag\/\(.*\)".*/\1/p'` 

	print_info "download mirror from $github_lz4_mirror"
	print_info "lz4 version : $LZ4_VERSION"
#	press_any_key
    curl -SL "https://github.com/squidproxy/anyconnect/archive/$LZ4_VERSION.tar.gz" -o lz4.tar.gz
...
function help_ocservauto(){
    ...
    echo
    print_info " upgrade ocserv version----------------- Smoothly upgrade your ocserv"
    echo
    print_info " reinstall --------------- Force to reinstall your ocserv(Destroy All Data)"
    echo
...
NET_OC_CONF_DOC="https://raw.githubusercontent.com/squidproxy/eazy-for-ss/master/ocservauto"
#推荐的默认版本
...
*)
    ...
    print_warn "Usage:  bash `basename $0` {install|fm|gc|rc|update|reinstall|pc|occ|help}"
    help_ocservauto
    ;;
esac
exit 0